...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if ($driver === 'mysql') {
            ...
            DB::statement('ALTER TABLE beneficiaries ADD COLUMN temp_employee_id VARCHAR(255) NULL AFTER member_profile_id');
            DB::statement('UPDATE beneficiaries b INNER JOIN member_profiles mp ON b.member_profile_id = mp.id SET b.temp_employee_id = mp.employee_id');

            Schema::table('beneficiaries', function (Blueprint $table) {
                $table->dropColumn('member_profile_id');
            });

            DB::statement('ALTER TABLE beneficiaries CHANGE temp_employee_id member_profile_id VARCHAR(255) NULL');

            Schema::table('beneficiaries', function (Blueprint $table) {
                $table->foreign('member_profile_id', 'beneficiaries_member_profile_id_foreign')
                    ->references('employee_id')->on('member_profiles')
                    ->cascadeOnDelete();
            });
            ...
            DB::statement('ALTER TABLE deduction_records ADD COLUMN temp_employee_id VARCHAR(255) NULL AFTER member_profile_id');
            DB::statement('UPDATE deduction_records dr INNER JOIN member_profiles mp ON dr.member_profile_id = mp.id SET dr.temp_employee_id = mp.employee_id WHERE dr.member_profile_id IS NOT NULL');

            Schema::table('deduction_records', function (Blueprint $table) {
                $table->dropColumn('member_profile_id');
            });

            DB::statement('ALTER TABLE deduction_records CHANGE temp_employee_id member_profile_id VARCHAR(255) NULL');
            ...
            DB::statement('ALTER TABLE payroll_upload_rows ADD COLUMN temp_employee_id VARCHAR(255) NULL AFTER matched_member_profile_id');
            DB::statement('UPDATE payroll_upload_rows pur INNER JOIN member_profiles mp ON pur.matched_member_profile_id = mp.id SET pur.temp_employee_id = mp.employee_id WHERE pur.matched_member_profile_id IS NOT NULL');

            Schema::table('payroll_upload_rows', function (Blueprint $table) {
                $table->dropColumn('matched_member_profile_id');
            });

            DB::statement('ALTER TABLE payroll_upload_rows CHANGE temp_employee_id matched_member_profile_id VARCHAR(255) NULL');
            ...
            // 4. member_profiles: drop id PK and column, make employee_id PK
            ...
            Schema::create('temp_member_profile_mapping', function (Blueprint $table) {
                ...
                $table->primary(['old_id', 'employee_id']);
            });

            DB::statement('INSERT INTO temp_member_profile_mapping SELECT id, employee_id FROM member_profiles');
            ...
            DB::statement('INSERT INTO beneficiaries_new (id, member_profile_id, full_name, relationship, date_of_birth, created_at, updated_at) SELECT b.id, mp.employee_id, b.full_name, b.relationship, b.date_of_birth, b.created_at, b.updated_at FROM beneficiaries b INNER JOIN member_profiles mp ON b.member_profile_id = mp.id');
            ...
            DB::statement('INSERT INTO payroll_upload_rows_new (id, payroll_upload_id, matched_user_id, matched_member_profile_id, row_number, employee_id, payroll_id, member_id, employee_name, cutoff_date, deduction_amount, applied_amount, unapplied_amount, status, deduction_status, errors, raw_payload, remarks, processed_at, created_at, updated_at) SELECT pur.id, pur.payroll_upload_id, pur.matched_user_id, mp.employee_id, pur.row_number, pur.employee_id, pur.payroll_id, pur.member_id, pur.employee_name, pur.cutoff_date, pur.deduction_amount, pur.applied_amount, pur.unapplied_amount, pur.status, pur.deduction_status, pur.errors, pur.raw_payload, pur.remarks, pur.processed_at, pur.created_at, pur.updated_at FROM payroll_upload_rows pur LEFT JOIN member_profiles mp ON pur.matched_member_profile_id = mp.id');
