...
const breadcrumbs: BreadcrumbItem[] = [
    {
        title: 'Credit Coordinator Dashboard',
        href: dashboard().url,
    },
];

// Types for Credit Coordinator Dashboard props
interface CreditComStats {
...
interface CreditComDashboardProps {
    stats: CreditComStats;
    recent_pending_loans: RecentPendingLoan[];
    loan_health: LoanHealth;
}

export default function CreditComDashboard({ 
    stats, 
    recent_pending_loans, 
    loan_health
}: CreditComDashboardProps) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs} headerRight={<LiveClock />}>
            <Head title="Credit Coordinator Dashboard" />
            
            <div className="flex flex-1 flex-col gap-6 p-6">
                {/* Navigation Cards */}
                <div className="grid gap-4 md:grid-cols-2">
                    <Link href="/dashboards/CreditCom/LoanApplication">
                        <Card className="border-emerald-100 bg-white/50 dark:bg-emerald-950/10 shadow-sm hover:shadow-md transition-shadow cursor-pointer h-full">
                            <CardHeader className="flex flex-row items-center justify-between pb-2">
                                <div>
                                    <CardTitle className="text-lg font-medium text-emerald-800 dark:text-emerald-300">Loan Applications</CardTitle>
                                    <CardDescription className="mt-1">View all pending loan applications</CardDescription>
                                </div>
                                <FileText className="h-8 w-8 text-emerald-600" />
                            </CardHeader>
                            <CardContent>
                                <div className="text-3xl font-bold text-emerald-700">{stats.pending_approvals}</div>
                                <p className="text-sm text-muted-foreground mt-1">Pending applications</p>
                            </CardContent>
                        </Card>
                    </Link>

                    <Link href="/dashboards/CreditCom/ValidateLoan">
                        <Card className="border-blue-100 bg-white/50 dark:bg-blue-950/10 shadow-sm hover:shadow-md transition-shadow cursor-pointer h-full">
                            <CardHeader className="flex flex-row items-center justify-between pb-2">
                                <div>
                                    <CardTitle className="text-lg font-medium text-blue-800 dark:text-blue-300">Validate Applications</CardTitle>
                                    <CardDescription className="mt-1">Review and approve loan applications</CardDescription>
                                </div>
                                <ClipboardCheck className="h-8 w-8 text-blue-600" />
                            </CardHeader>
                            <CardContent>
                                <div className="text-3xl font-bold text-blue-700">{stats.pending_approvals}</div>
                                <p className="text-sm text-muted-foreground mt-1">Applications to validate</p>
                            </CardContent>
                        </Card>
                    </Link>
                </div>
                    ...
                    <Card className="border-emerald-600 bg-emerald-600 text-white shadow-md shadow-emerald-200 dark:shadow-none">
                        ...
                        <CardContent>
                            ...
                            <p className="text-xs text-emerald-100 mt-1 italic">
                                {stats.pending_approvals > 0 ? 'Action required' : 'No pending validations'}
                            </p>
                        </CardContent>
                    </Card>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
                    ...
                    <Card className="lg:col-span-4 border-emerald-100">
                        <CardHeader className="flex flex-row items-center justify-between">
                            <div>
                                <CardTitle className="text-emerald-900 dark:text-emerald-100">Applications for Validation</CardTitle>
                                <CardDescription>Loan applications approved by GM awaiting your review.</CardDescription>
                            </div>
                        ...
                        <CardContent>
                            {recent_pending_loans.length > 0 ? (
                                ...
                                <div className="text-center py-8 text-muted-foreground">
                                    ...
                                    <p className="font-medium">No pending validations</p>
                                    <p className="text-sm">All loan applications have been validated</p>
                                </div>
                            )}
                        </CardContent>
                    </Card>
